  ...
  try {
    ...
    <div style="--mt:2rem;--d:grid;--gap:2rem;" class="with-anchor-lines" data-replies>
      ${replies.map(mention => outdent`
        <div style="--d:grid;--gap:1rem;--bg:rgb(100 100 100/.06);--p:2rem;--roundness:.5rem;">
          <div style="--d:flex;--justify_content:space-between;--align_items:center;--opacity:.6;">
            <div>
          ...
          ${mention.content?.text.length < 400 ? `<div style="--word_break:break-word;">${mention.content.html}</div>` : ''}
          <div style="--font_sz:1.4rem;--word_break:break-all;">
